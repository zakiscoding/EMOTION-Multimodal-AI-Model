...
def parse_args():
    ...
    parser.add_argument('--dev_dir', type=str, default=SM_CHANNEL_VALIDATION)
...
def main():
    ...
    train_loader, val_loader, test_loader = prepare_dataloaders(
        train_csv=os.path.join(arg.train_dir, 'train_sent_emo.csv'),
        train_video_dir=os.path.join(arg.train_dir, 'train_splits'),
        dev_csv=os.path.join(arg.dev_dir, 'dev_sent_emo.csv'),
        dev_video_dir=os.path.join(arg.dev_dir, 'dev_splits_complete'),
        test_csv=os.path.join(arg.test_dir, 'test_sent_emo.csv'),
        test_video_dir=os.path.join(arg.test_dir, 'output_repeated_splits_test'),
        batch_size= arg.batch_size
    )
    
    print(f'''Traiing DSV path: {os.path.join(arg.train_dir, "train_sent_emo.csv")}''')
    print(f'''Train video directory: {os.path.join(arg.train_dir, "train_splits")}''')
    ...
    for epoch in tqdm(range(arg.epochs), desc='Epochs'):
